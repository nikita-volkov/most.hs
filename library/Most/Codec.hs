...
sum3 (Codec enc1 dec1) (Codec enc2 dec2) (Codec enc3 dec3) =
  Codec
    ( \case
...
sum4 (Codec enc1 dec1) (Codec enc2 dec2) (Codec enc3 dec3) (Codec enc4 dec4) =
  Codec
    ( \case
        Left a -> Put.putWord8 0 *> enc1 a
        Right (Left a) -> Put.putWord8 1 *> enc2 a
...
sum5 (Codec enc1 dec1) (Codec enc2 dec2) (Codec enc3 dec3) (Codec enc4 dec4) (Codec enc5 dec5) =
  Codec
    ( \case
        Left a -> Put.putWord8 0 *> enc1 a
        Right (Left a) -> Put.putWord8 1 *> enc2 a
...
varLengthUnsignedIntegral =
  Codec
    ( let loop !state =
            let nextState = unsafeShiftR state 7
             in if nextState == 0
    ...
    ( let loop !index !state = do
            byte <- Get.getWord8
            if testBit byte 7
              then
                let nextState = unsafeShiftL (fromIntegral (clearBit byte 7)) index .|. state
...
integer =
  Codec
    ( let start negative state =
            let nextState = unsafeShiftR state 6
...
foldable length unfoldr fold (Codec enc dec) =
  Codec
    ( \a -> do
...
intMap keyCodec valueCodec =
  foldable
    (fromIntegral . IntMap.size)
    Unfoldr.intMapAssocs
    (const Folds.intMap)
...
vector codec =
  Codec
    ...
    ( do
        ...
        let !mvec = unsafeDupablePerformIO (MVec.unsafeNew length)
            loop index =
              if index < length
                then do
                  a <- get codec
